...
def get_audio(*data, streaming=False):
    ...
    try:
        ...
        # 取得角色名稱並清理非法字符
        character_name = task.character.replace("【", "").replace("】", "").replace(" ", "_")
        character_name = "".join(c if c.isalnum() or c in "_-" else "_" for c in character_name)

        # 取得前 20 個字符作為檔名
        raw_text = data.get("text", "output")[:20].replace(" ", "_")
        raw_text = "".join(c if c.isalnum() or c in "_-" else "_" for c in raw_text)
        filename = f"{raw_text}.{task.format}"

        save_path = tts_synthesizer.generate(task, return_type="filepath")

        # 建立角色專屬的資料夾
        output_dir = os.path.join("output", character_name)
        os.makedirs(output_dir, exist_ok=True)

        # 目標檔案路徑
        new_save_path = os.path.join(output_dir, filename)

        # ✅ 如果檔案已存在，自動加上 _1, _2, _3...
        counter = 1
        base_filename, ext = os.path.splitext(new_save_path)
        while os.path.exists(new_save_path):
            new_save_path = f"{base_filename}_{counter}{ext}"
            counter += 1

        os.rename(save_path, new_save_path)

        return new_save_path, new_save_path  # 第一個給試聽，第二個給下載
...
default_text = i18n("1.本模型所用语音等等的所有权均归米哈游所有。只可用于二次创作。不得创作任何违反法律法规的内容，不得用于任何商业用途，不得二次配布。如有滥用行为，该模型将永久停止公开！2. 发视频请注明模型训练者、整合包作者和数据集整理者。")
    ...
    with gr.Tabs():
        with gr.Tab(label=i18n("请求完整音频")):
            with gr.Row():
                ...
                full_audio = gr.Audio(label="試聽音頻", type="filepath", streaming=False)
                download_audio = gr.File(label="下載音頻", type="filepath")
                get_full_audio_button.click(
                    get_audio,
                    inputs=[value for key, value in all_gradio_components.items()],
                    outputs=[full_audio, download_audio],  # 第一個給試聽，第二個給下載
                )
