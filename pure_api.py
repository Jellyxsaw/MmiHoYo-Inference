...
async def tts(request: Request):
    ...
    if task.stream == False:
        ...
        if task.save_temp and md5_value in temp_files:
            ...
            text_based_filename = f"{task.text[:20].replace(' ', '').replace('。', '')}.{task.format}"  # 取前10個字，避免過長
            print(text_based_filename)

            try:
                return FileResponse(save_path, media_type=f"audio/{task.format}", filename=text_based_filename)
            except:
                return FileResponse(save_path, media_type=f"audio/{task.format}")
    else:
